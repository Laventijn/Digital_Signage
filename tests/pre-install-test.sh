...
test_config() {
  ...
  local key value
  for key in CONTENT_MODE PRESENTATION_URL SCREENSHOT_CACHE_ENABLED SCREENSHOT_CACHE_REFRESH_SECONDS SCREENSHOT_CAPTURE_WIDTH SCREENSHOT_CAPTURE_HEIGHT SCREENSHOT_CAPTURE_DEBUG_PORT SCREENSHOT_STABLE_GAP_MS SCREENSHOT_CHANGE_POLL_MS SCREENSHOT_TRANSITION_WAIT_MS SCREENSHOT_MAX_SLIDES SCREENSHOT_MAX_CAPTURE_SECONDS SCREENSHOT_MAX_CONSECUTIVE_FAILURES SCREENSHOT_DEBUG_STABILITY SCREENSHOT_SINGLE_SLIDE_CONFIRM_SECONDS SCREENSHOT_IMAGE_DIFFERENCE_PERCENT SCREENSHOT_CACHE_KEEP_VERSIONS OFFLINE_WATERMARK_TEXT WEBSITE_OFFLINE_CAPTURE_MODE OFFLINE_URL CHROMIUM_BIN WAYLAND_DISPLAY REMOTE_DEBUG_HOST REMOTE_DEBUG_PORT CACHE_SIZE_MB KIOSK_USER REFRESH_SECONDS SWAP_LOG_MAX_BYTES RESOURCE_LOG_RETENTION_DAYS HEALTH_CHECK_SECONDS HEALTH_FAILURE_THRESHOLD HEALTH_RESTART_COOLDOWN_SECONDS HEALTH_HTTP_TIMEOUT_SECONDS HEALTH_STARTUP_GRACE_SECONDS HEALTH_LOG_RETENTION_DAYS HEALTH_LOG_MAX_BYTES DESKTOP_BACKGROUND_ENABLED DESKTOP_BACKGROUND_FILE DESKTOP_BACKGROUND_MODE; do
